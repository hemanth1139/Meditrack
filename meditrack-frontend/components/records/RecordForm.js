...
export default function RecordForm({ initialPatientId, onSubmit, submitting, showPendingInfo }) {
  ...
  // Hook up record_type secretly to react-hook-form so validation works
  useEffect(() => { register("record_type", { required: true }); }, [register]);

  const submit = async (values) => {
    if (!values.patient_id) return toast.error("Patient ID is required");
    if (!values.record_type) return toast.error("Record type is required");
    ...
    <form className="grid gap-4" onSubmit={handleSubmit(submit, () => toast.error("Please fill all required fields"))}>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <div className="grid gap-2">
          ...
          <input type="hidden" {...register("record_type", { required: "Record type is required" })} />
          <Select 
            value={watch("record_type")} 
            onValueChange={(v) => setValue("record_type", v, { shouldValidate: true })}
          >
            <SelectTrigger>
          ...
          {errors.record_type && <span className="text-xs text-red-500">{errors.record_type.message}</span>}
        </div>
        <div className="grid gap-2">
