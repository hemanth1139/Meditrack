...
const InputField = ({ label, error, children }) => (
  <motion.div variants={itemVariants} className="flex flex-col gap-1.5">
    ...
    {error && (
      <motion.span initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="text-[12px] font-medium text-red-500">
        {error}
      </motion.span>
    )}
  </motion.div>
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-slate-50 font-sans px-4 py-8">
      ...
      >
        ...
        <motion.div variants={itemVariants} className="mb-8 flex overflow-x-auto border-b-2 border-slate-200/80">
          <AnimatePresence>
            {TAB_ITEMS.map((tab) => {
          ...
          {activeTab === "patient" && (
            <form onSubmit={patientForm.handleSubmit(onSubmitPatient)} className="space-y-4">
              ...
              <motion.div variants={itemVariants} className="pt-4">
                <motion.button
                  whileHover={{ boxShadow: "0 10px 25px -5px rgba(37, 99, 235, 0.4)" }}
                  type="submit"
                ...
                {doctorSuccess ? (
                  ...
                  >
                    ...
                    <motion.div variants={itemVariants} className="pt-2">
                      ...
                      <motion.div
                        whileHover={{ scale: 1.01, backgroundColor: "rgba(248, 250, 252, 1)", borderColor: "#93c5fd" }}
        ...
        <motion.div variants={itemVariants} className="mt-8 text-center text-[14px] font-medium text-slate-500">
