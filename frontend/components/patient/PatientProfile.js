        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {role === "STAFF" && (
                <>
                  <Button onClick={() => setAddVitalsOpen(true)} className="bg-white/10 text-white hover:bg-white/20 border-0">
           ...
           {vitals?.length > 0 ? (
             <>
              <Card className="p-6 border-slate-200">
                ...
                <div className="w-full flex gap-1 h-32 items-end mt-4">
                  {[...vitals].reverse().map((v, idx) => {
                    ...
                    return (
                       <div key={idx} className="flex-1 flex flex-col items-center gap-1 group relative">
                         <div className="absolute bottom-[110%] w-max bg-white border border-slate-200 shadow-sm rounded-md px-2 py-1 text-xs text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none">
                           {formatDate(v.recorded_at).split(',')[0]}<br/>
                           BP: {v.blood_pressure} | HR: {hr}
                         </div>
                         <div className="flex gap-1 items-end w-full justify-center h-[100px]">
                           <div className="w-1/3 bg-rose-500 rounded-t-sm max-w-[12px]" style={{height: `${Math.min((sys / maxSys)*100, 100)}%`}} />
                           <div className="w-1/3 bg-blue-500 rounded-t-sm max-w-[12px]" style={{height: `${Math.min((hr / maxHr)*100, 100)}%`}} />
                         </div>
                       </div>
                    );
                  })}
                </div>
                <div className="flex justify-center mt-6 gap-6 text-[11px] font-semibold text-slate-500 mb-2">
                  <div className="flex items-center gap-1.5"><div className="w-3 h-3 bg-rose-500 rounded-sm"></div> Systolic BP</div>
                  <div className="flex items-center gap-1.5"><div className="w-3 h-3 bg-blue-500 rounded-sm"></div> Heart Rate</div>
                </div>
              </Card>
