...
export default function HospitalApprovalTable({ pendingDoctors, hospitalId }) {
  ...
  const handleApprove = async (doctor) => {
    ...
    try {
      await api.post(`/users/doctors/${doctor.id}/approve/`, { hospital_id: hospitalId });
  ...
  const handleReject = async () => {
    ...
    setActionLoading(true);
    try {
      await api.post(`/users/doctors/${rejectTarget.id}/reject/`, { reason: rejectReason, hospital_id: hospitalId });
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mt-6">
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        ...
        <Link href="/dashboard/hospital-admin/doctors" className="text-sm text-blue-500 hover:underline font-medium">
          View All →
        </Link>
      </div>
